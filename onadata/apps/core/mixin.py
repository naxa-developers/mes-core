...
class ProjectView(LoginRequiredMixin):
    ...
    def get_queryset(self):
        if 'project_id' in self.request.session:
            project = Project.objects.get(id=self.request.session['project_id'])
            return super(ProjectView, self).get_queryset().filter(project=project)
        elif self.request.project:
    ...
    def get_form(self, *args, **kwargs):
        ...
        if hasattr(form.Meta, 'project_filters'):
            for field in form.Meta.project_filters:
                if 'project_id' in self.request.session:
                    project = Project.objects.get(id=self.request.session['project_id'])
                    form.fields[field].queryset = Project.objects.filter(id=project.pk)
                elif self.request.project:
...
class ProjectMixin(object):
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated():
            if UserRole.objects.filter(project__id=self.request.session.get('project_id'), user=self.request.user).exists():
                roles = UserRole.objects.filter(project__id=self.request.session.get('project_id'), user=self.request.user)
                role = roles[0]
                if role.group.name in USER_PERMS['project']:
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated():
            if UserRole.objects.filter(project__id=self.request.session.get('project_id'), user=self.request.user).exists():
                roles = UserRole.objects.filter(project__id=self.request.session.get('project_id'), user=self.request.user)
                role = roles[0]
                if role.group.name in USER_PERMS['manager']:
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated():
            if UserRole.objects.filter(project__id=self.request.session.get('project_id'), user=self.request.user).exists():
                roles = UserRole.objects.filter(project__id=self.request.session.get('project_id'), user=self.request.user)
                role = roles[0]
                if role.group.name in USER_PERMS['admin']:
